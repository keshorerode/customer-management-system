...
export default function ProductsPage() {
  ...
  const createProduct = useMutation({
    ...
    onSuccess: () => {
      ...
      setFormData({ name: "", code: "", price: "", category: "Software", description: "" });
    },
    onError: (err: any) => {
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {products?.map((product) => (
            <div key={product.id} className="bg-bg-surface border border-border-main rounded-card hover:border-brand-primary/40 transition-all group overflow-hidden">
              <div className="p-6">
